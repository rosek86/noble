...
HciBle.prototype.onStdoutData = function(data) {
  ...
  while ((newLineIndex = this._buffer.indexOf('\n')) !== -1) {
    ...
    } else if ((found = line.match(/^scan (.*)$/))) {
      var scan = this.parseScanResults(found[1]);
      this.handleScanResults(scan);
    } else if ((found = line.match(/^error (.*)$/))) {
      console.log(line);
...
HciBle.prototype.parseScanResults = function(results) {
  ...
  if (scanResults.action === 'start') {
    scanResults.filter = parts[2];
    if (scanResults.result === 'failure') {
      scanResults.error = parts[3];
    }
  } else {
    if (scanResults.result === 'failure') {
      scanResults.error = parts[2];
    }
  }
...
HciBle.prototype.handleScanResults = function(results) {
  if (results.action === 'start') {
    this.onScanStart(new Error(results.error));
  } else {
    this.onScanStop(new Error(results.error));
  }
};

HciBle.prototype.onScanStart = function(err) {
  this.emit('scanStart', err);
};

HciBle.prototype.onScanStop = function(err) {
